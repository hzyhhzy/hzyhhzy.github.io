...
class DenseIntMap {
    static SHIFT = 16;
    static CHUNK_SIZE = 1 << DenseIntMap.SHIFT;
    static MASK = DenseIntMap.CHUNK_SIZE - 1;
    static PROMOTE_THRESHOLD = 8192;
    chunks = [];
    sparse = new Map;
    count = 0;
    denseMode = false;
    ...
    has(key) {
        if (this.sparse.size !== 0 && this.sparse.has(key))
            return true;
    ...
    set(key, value) {
        if (!this.denseMode) {
            ...
            if (!existed)
                this.count++;
            if (this.sparse.size >= DenseIntMap.PROMOTE_THRESHOLD)
                this.promoteDense();
            return this;
        }
        if (DenseIntMap.denseInteger(key) && DenseIntMap.denseInteger(value)) {
    ...
    project(remap) {
        ...
        for (const [oldId, newId] of remap) {
            ...
            if (representative === undefined)
                representativeByRoot.set(oldRoot, newId);
            else
                projected.union(representative, newId);
        }
        return projected;
    }
    merge(ds, increment) {
...
export class Core {
    ...
    matchWithWhnf(ast, pattern, regexp, context, skipExpand, res = {}) {
        if (pattern.type === "var" && pattern.name.match(regexp)) {
            ...
            if (!Core.exactEqual(ast, res[pattern.name])) {
                if (this.state.reuseRuleWhnf !== false)
                    this.matchWhnfEpoch++;
    ...
    registConstType(name, ast, reusePrechecked = false) {
        this.state.errormsg = [];
        const precheckedDefinition = this.state.precheckedDefinition;
        reusePrechecked = Boolean(reusePrechecked
        ...
        if (precheckedPublicType)
            this.reduce(precheckedPublicType, [], true);
        ...
        this.state.defTypes[name] = this.compactConstTypeCache(publicType, this.state.inferTable, this.state.bondVarRel);
        return ast;
    }
    compactConstTypeCache(typeAst, inferTable, bondVarRel) {
        ...
        const collectAst = (ast) => {
            if (!ast || typeof ast !== "object" || seenCollect.has(ast))
                return;
            seenCollect.add(ast);
            if (ast.bondVarId && isFinite(ast.bondVarId))
                ids.add(ast.bondVarId);
            if (ast.nodes) {
        ...
        for (const context of cachedInfer.list.values())
            collectContext(context);
        for (const value of Object.values(cachedInfer.rel))
            collectAst(value);
        ...
        const remap = new Map(orderedIds.map((oldId, index) => [oldId, index + 1]));
        const seenRemap = new WeakSet;
        const remapAst = (ast) => {
            if (!ast || typeof ast !== "object" || seenRemap.has(ast))
                return;
            seenRemap.add(ast);
            if (ast.bondVarId && isFinite(ast.bondVarId)) {
    ...
    pruneConstInferCache(typeAst, inferTable) {
        const relevant = new Set;
        const pending = [];
        const seenAst = new WeakSet;
        const normalizeName = (name) => name?.match(/^\?([^\:]+):*$/)?.[1] ?? null;
        const addName = (name) => {
        ...
        const scanAst = (ast) => {
            if (!ast || typeof ast !== "object" || seenAst.has(ast))
                return;
            seenAst.add(ast);
            addName(ast.name);
            if (ast.nodes) {
        ...
        for (let index = 0; index < pending.length; index++) {
            ...
            if (context)
                scanContext(context);
            for (const value of relByName.get(name) ?? [])
                scanAst(value);
        }
        for (const name of [...inferTable.list.keys()]) {
            if (!relevant.has(name))
                inferTable.list.delete(name);
        }
