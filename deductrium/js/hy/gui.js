...
export class HyperGui {
    ...
    mapModeButton = document.getElementById("map-mode-toggle");
    recenterCurrentButton = document.getElementById("recenter-current");
    ...
    constructor() {
        ...
        this.canvas.addEventListener("mousemove", event => {
            if (this.dragging && (event.buttons & 1)) {
                ...
                if (this.world.isFlatMode()) {
                    ...
                    this.world.rotate(Math.atan2(this.lastPointerY - centerY, this.lastPointerX - centerX)
            ...
            if (!this.dragging)
                this.canvas.style.cursor = result.clickable ? "pointer" : "grab";
            if (result.changed)
                this.needUpdate = true;
        });
        window.addEventListener("mouseup", event => {
            ...
            if (wasClick && this.world.isFlatMode()) {
                ...
                this.world.activateAt((event.clientX - rectangle.left) * window.devicePixelRatio, (event.clientY - rectangle.top) * window.devicePixelRatio);
            }
            this.needUpdate = true;
        });
        ...
        this.canvas.addEventListener("touchstart", event => {
            ...
            if (this.world.isFlatMode() && event.targetTouches.length === 2) {
                this.touchDistance = Math.hypot(event.targetTouches[0].clientX - event.targetTouches[1].clientX, event.targetTouches[0].clientY - event.targetTouches[1].clientY);
                this.dragging = false;
                return;
            }
            ...
            const rectangle = this.canvas.getBoundingClientRect();
            this.outside = !this.world.isFlatMode() && this.world.localDraw.hitTestPoincareDisk(touch.clientX - rectangle.left, touch.clientY - rectangle.top);
        }, { passive: false });
        this.canvas.addEventListener("touchmove", event => {
            ...
            if (this.world.isFlatMode() && event.targetTouches.length === 2) {
                ...
                if (this.touchDistance)
                    this.world.zoomAt(distance / this.touchDistance, (first.clientX + second.clientX) / 2 - rectangle.left, (first.clientY + second.clientY) / 2 - rectangle.top);
                this.touchDistance = distance;
                this.needUpdate = true;
                return;
            }
            const touch = event.targetTouches[0];
            if (!touch || !this.dragging)
                return;
        ...
        this.canvas.addEventListener("touchend", event => {
            ...
            if (this.world.isFlatMode() && this.dragging && this.dragDistance < 7) {
                const rectangle = this.canvas.getBoundingClientRect();
                this.world.activateAt((this.pointerStartX - rectangle.left) * window.devicePixelRatio, (this.pointerStartY - rectangle.top) * window.devicePixelRatio);
            }
            this.dragging = false;
    ...
    focusCurrentNode() {
        if (this.world.focusCurrentNode(32))
            this.needUpdate = true;
    }
    onresize() {
