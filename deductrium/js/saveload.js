...
export class GameSaveLoad {
    storageKey = survivalStorageKey;
    constructor(gamemode) {
        if (gamemode === "creative") {
            this.storageKey = creativeStorageKey;
    ...
    serialize(game) {
        const selectedTimeout = document.getElementById("timeSelect")?.value;
    ...
    deserialize(game, data) {
        ...
        let consumed;
        let destructedGates;
        let maxOrd, ordBase, timeoutSeconds;
        [
    ...
    serializeStr(json) {
        ...
        let randomFunction = new Rnd(this.storageKey === survivalStorageKey ? json.length : (json.length - 1));
        // console.log(json);
        const l78z = Shuffle.shuffleArray(Array.from(json), randomFunction);
        return l78z.join("");
    }
    deserializeStr(str) {
        let randomFunction = new Rnd(this.storageKey === survivalStorageKey ? str.length : (str.length - 1));
