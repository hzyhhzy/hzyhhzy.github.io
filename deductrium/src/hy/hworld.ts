...
import { FlatMapModel } from "./flatmap.js";
...
function boundaryIntersection(a, b, bounds) {
    ...
    const dy = b.y - a.y;
    const candidates = [];
    if (dx > 0) candidates.push((bounds.right - a.x) / dx);
    if (dx < 0) candidates.push((bounds.left - a.x) / dx);
    if (dy > 0) candidates.push((bounds.bottom - a.y) / dy);
    if (dy < 0) candidates.push((bounds.top - a.y) / dy);
...
function polylineFromRatio(points, ratio, towardEnd) {
    ...
    if (towardEnd) return [cursor, ...points.slice(Math.floor(position) + 1)];
...
export class HWorld {
    ...
    prettyPrint = true;
    highLightGetD = false;
    onPassGate: (name: string, tile: TileBlock, hash: string) => boolean = () => false;
    ...
    sourceRoadSnapshot: {
        ...
        passiveRoadHashes: Set<string>,
        emptyRoadHashes: Set<string>,
    } = null;
    geometryMode: "flat" | "hyperbolic" = "flat";

    flatModel: FlatMapModel = null;
    graphDirty = true;
    zoom = 1;
    panX = 0;
    panY = 0;
    ...
    captureSourceRoadSnapshot() {
        const nodeHashes = new Set<string>(blockMap.keys());
        const passiveRoadHashes = new Set<string>();
        const emptyRoadHashes = new Set<string>();
        for (const [hash, block] of blockMap) {
            if (block.type !== TileBlockType.Road || block.name) continue;
            passiveRoadHashes.add(hash);
            if (!block.text?.trim()) emptyRoadHashes.add(hash);
        }
    ...
    setGeometryMode(mode: "flat" | "hyperbolic") {
        if (mode === this.geometryMode) return mode;
        this.geometryMode = mode;
        if (mode === "flat") this.overviewMode = true;
    ...
    ensureModel() {
        ...
        if (this.overviewMode) this.flatModel.ensureGlobalLayout();
    ...
    drawRoute(context, edge, isHighlighted) {
        ...
        if (control) context.quadraticCurveTo(control.x, control.y, endpoints[1].x, endpoints[1].y);
        else context.lineTo(endpoints[1].x, endpoints[1].y);
    ...
    fitNodeLines(context, text, maximumWidth, maximumLines = 4) {
        ...
        if (index < characters.length && lines.length) {
            ...
            while (last.length && context.measureText(last.join("") + "…").width > maximumWidth) last.pop();
    ...
    drawNode(context, entry, clickable, radius, highlighted = false) {
        ...
        if (this.highLightGetD && block?.text?.match(/^获取(.+)推理素$/)) fill = "#ffe64a";
        context.save();
    ...
    drawCurrentCursor(context, routeScreens, radius) {
        ...
        if (!location || location.kind === "node") return;
        let point = null;
        if (location.kind === "edge") {
            ...
            if (coord) point = this.worldToScreen(coord);
        }
        if (!point) return;
        const dpr = window.devicePixelRatio;
        context.save();
    ...
    wrappedLines(context, text, maximumCharacters = 28, maximumLines = 3) {
        ...
        if (sourceLength > result.join("").length) result[result.length - 1] = result[result.length - 1].replace(/…$/, "") + "…";
        return result;
    }
    ...
    drawLabels(context, clickableHashes) {
        ...
        for (const node of candidates) {
            ...
            if (!important && (this.overviewMode || this.zoom < 0.42 || drawn >= 28)) continue;
            ...
            for (const placement of placements) {
                ...
                if (rectangle.x < 4 * dpr || rectangle.y < 4 * dpr || rectangle.x + width > canvas.width - 4 * dpr || rectangle.y + height > canvas.height - 4 * dpr) score += 5000;
                if (score < bestScore) {
                    best = rectangle;
                    bestScore = score;
                }
            }
            if (!important && bestScore >= 10000) continue;
            used.push(best);
            context.save();
    ...
    drawFlatNotice(context, lines: string[], centered = false) {
        const dpr = window.devicePixelRatio;
        const canvas = this.localDraw.canvas;
    ...
    drawHoverCard(context) {
        ...
        if (mergedRoadCount > 1) lines.unshift(`已合并 ${mergedRoadCount} 个无功能路点`);
        if (block.name) lines.unshift(`[${block.name}]`);
        context.save();
    ...
    onLoop() {
        if (this.isFlatMode()) this.drawFlatWorld();
        else this.drawHyperbolicWorld();
    }
    ...
    drawHyperbolicWorld() {
        ...
        for (const [hash, rotor] of this.atlasTile.rotors) {
            ...
            if (this.highLightGetD && block?.text?.match(/^获取(.+)推理素$/)) context.fillStyle = "rgb(255,255,0)";
            else context.fillStyle = ["rgb(255,255,255)", "rgb(220,220,220)", "rgb(250,160,20)", "rgb(200,255,20)", "rgb(180,255,240)"][block?.type ?? TileBlockType.Wall];
    ...
    drawFlatWorld() {
        ...
        for (const edge of orderedEdges) routeScreens.set(edge.id, this.drawRoute(context, edge, highlightedEdges.has(edge.id)));
        ...
        for (const route of routes) {
            ...
            if (target) this.screenTargets.push(target);
        }
        ...
        for (const node of this.screenNodes) this.drawNode(context, node, clickableHashes.has(node.hash), nodeRadius,
            region.highlightedNodes.has(node.hash));
        this.drawCurrentCursor(context, routeScreens, nodeRadius);
        for (const target of this.screenTargets.filter(target => target.portal)) this.drawPortal(context, target);
        for (const target of this.screenTargets.filter(target => target.routeChoice)) this.drawRouteChoice(context, target);
        ...
        if (this.debugDraw) {
            ...
            context.textAlign = "left";
            for (const node of this.screenNodes) context.fillText(node.hash || "[]", node.x + 10 * dpr, node.y + 12 * dpr);
            context.restore();
        }
    ...
    hoverAt(x, y) {
        if (!this.isFlatMode()) return { changed: false, clickable: false };
    ...
    moveAlong(route) {
        ...
        if (movedHash !== oldCurrent) {
            ...
            if (this.overviewMode) {
                ...
                if (back) this.viewRotation = desiredBackAngle - Math.atan2(back.y, back.x);
                this.panX = 0;
                this.panY = 0;
            }
        }
    ...
    focusCurrentNode(targetZoom = 32) {
        if (!this.isFlatMode() || !this.overviewMode) return false;
        const model = this.ensureModel();
    ...
    toggleOverview() {
        if (!this.isFlatMode()) return this.overviewMode;
        this.overviewMode = !this.overviewMode;
        if (this.overviewMode) this.ensureModel().ensureGlobalLayout();
        this.hoveredNode = null;
        this.hoveredEdge = null;
        this.resetView();
    ...
    hitTest(tile) {
        ...
        const block = this.getBlock(hash);
        if (!block || block.type === TileBlockType.Wall) return false;
        if (block.type === TileBlockType.Road) return true;
        if (block.type === TileBlockType.Reward) {
            if (block.text) this.hitReward(block, hash);
            return true;
        }
    ...
    hitReward(block: TileBlock, hash: string, isLoading?: boolean) {
        if (!block) return;
        this.onGetReward(block.name ?? hash, block, isLoading);
        if (block.type !== TileBlockType.Gate) block.text = "已" + block.text;
    ...
    moveHyperbolicCamera(x: number, y: number) {
    ...
    rotate(angle) {
        if (this.isFlatMode()) this.viewRotation += angle;
        else this.localCamMat = Rotor.rotate(angle).mul(this.localCamMat).normalize();
    }
